...
| Expo runtime | Mobile platform integration | Runs the current mobile application | Local Expo configuration | Medium; planned for removal | `apps/mobile/package.json`, `apps/mobile/index.ts` |
...
- Hardcoding check: the mobile API base URL is hard-coded to a private local network address; no committed secret value was found in the inspected source files.
...
- CORS: Payload allows `http://localhost:3001`, `http://localhost:3002`, and `exp://*` in the current configuration.
