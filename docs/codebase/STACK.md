...
| React | `19.2.6` catalog; mobile declares `19.1.0` | User-interface runtime | `package.json`, `apps/mobile/package.json` |
...
| TanStack Query (`@tanstack/react-query`) | `^5.62.11` catalog | Client-side query and cache layer in web and mobile applications | `package.json`, `apps/web/package.json`, `apps/web/lib/hooks.ts`, `apps/mobile/lib/hooks.ts` |
...
| Expo `~54.0.34` | Current mobile development runtime; planned for removal | `apps/mobile/package.json` |
...
- Mobile API URL: currently hard-coded as `http://10.0.0.145:3000/api` in `apps/mobile/lib/api.ts`.
